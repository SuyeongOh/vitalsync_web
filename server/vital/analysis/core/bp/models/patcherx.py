...
class Backbone(nn.Module):
    def __init__(self, configs):
        ...
        self.seq_len = seq_len = 600  # configs.seq_len  # default 336 / input sequence length
        self.pred_len = pred_len = 1  # configs.pred_len  # default 336 / prediction sequence length

        # Patching
        self.patch_len = patch_len = 16  # configs.patch_len  # 16 / patch length
        self.stride = stride = 8  # configs.stride  # 8 / stride
        self.patch_num = patch_num = int((seq_len - patch_len) / stride + 1)
        self.padding_patch = 'end'  # configs.padding_patch  # 'end' / None: None; end: padding on the end
...
class Model(nn.Module):
    ...
    def __init__(self, configs=None):
        ...
        self.rev = RevIN(num_features=4)  # encoder input : Default 7 / channel or dimension

        self.backbone = Backbone(configs)

        self.seq_len = 600  # configs.seq_len  # Default 336 / input sequence length
        self.pred_len = 1  # configs.pred_len  # Default 336 / prediction sequence length

        self.signal_zip = Mlp(4, 2, 1)

        # 나이와 성별 정보를 처리하기 위한 추가 레이어
        self.mlp = Mlp(5, 4, 1)

    def forward(self, x, hr, age, gender):  # , batch_x_mark, dec_inp, batch_y_mark):
        z = self.rev(x, 'norm')  # B, L, D -> B, L, D
        z = self.backbone(z)  # B, L, D -> B, H, D
        z = self.rev(z, 'denorm')  # B, L, D -> B, H, D
        z = self.signal_zip(z).squeeze(-1)

        # age_embed = self.age_embed(age)
        # gender_embed = self.gender_embed(gender)
        z = torch.cat([z, hr, age, gender], dim=-1)
        z = self.mlp(z)
        return z.squeeze(-1)
